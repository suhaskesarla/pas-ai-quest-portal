...
public sealed class ChallengeAdministrationTests : IAsyncLifetime
{
    ...
    public async Task InitializeAsync() { db = Context(); await db.Database.MigrateAsync(); db.Participants.AddRange(new Participant { Id = manager, DisplayName = "Synthetic Manager", CreatedAt = clock.GetUtcNow() }, new Participant { Id = participant, DisplayName = "Synthetic Participant", CreatedAt = clock.GetUtcNow() }); db.Cycles.Add(new Cycle { Id = cycle, Code = "ADMIN-26", Name = "Administration Cycle", Status = CycleStatus.Active, StartsAt = clock.GetUtcNow().AddDays(-1), EndsAt = clock.GetUtcNow().AddMonths(1), CreatedAt = clock.GetUtcNow(), CreatedByParticipantId = manager }); db.CycleParticipants.Add(new CycleParticipant { CycleId = cycle, ParticipantId = participant, Status = CycleParticipantStatus.Active }); await db.SaveChangesAsync(); }
