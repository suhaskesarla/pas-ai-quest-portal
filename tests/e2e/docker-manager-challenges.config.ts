...
process.env.QA_SCREENSHOT_DIR = path.join(runRoot, 'docker-manager-challenges')
process.env.QA_TEST_MODE = 'Docker Compose Manager Challenge Administration'
...
export default defineConfig({
  testDir: '.',
  testMatch: 'docker-manager-challenges.spec.ts',
  timeout: 90_000,
  workers: 1,
  fullyParallel: false,
  reporter: [['list'], ['./evidence-reporter.ts']],
  outputDir: path.join(runRoot, 'docker-manager-challenges', 'artifacts'),
