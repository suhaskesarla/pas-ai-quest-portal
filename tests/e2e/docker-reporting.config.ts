...
process.env.QA_SCREENSHOT_DIR = path.join(runRoot, 'docker-reporting')
process.env.QA_TEST_MODE = 'Docker Compose Participant Reporting'
...
export default defineConfig({
  testDir: '.',
  testMatch: 'docker-reporting.spec.ts',
  timeout: 60_000,
  workers: 1,
  fullyParallel: false,
  reporter: [['list'], ['./evidence-reporter.ts']],
  outputDir: path.join(runRoot, 'docker-reporting', 'artifacts'),
