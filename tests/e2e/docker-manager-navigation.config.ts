...
process.env.QA_SCREENSHOT_DIR = path.join(runRoot, 'docker-manager-navigation')
process.env.QA_TEST_MODE = 'Docker Compose Manager Navigation Cleanup'
process.env.QA_BASE_URL = 'http://localhost:5173'
process.env.QA_CLEAN_DATABASE = 'true'
process.env.QA_FIXTURE_DATA = 'false'

export default defineConfig({
  testDir: '.', testMatch: 'docker-manager-navigation.spec.ts', timeout: 90_000,
  workers: 1, fullyParallel: false,
  reporter: [['list'], ['./evidence-reporter.ts']],
  outputDir: path.join(runRoot, 'docker-manager-navigation', 'artifacts'),
