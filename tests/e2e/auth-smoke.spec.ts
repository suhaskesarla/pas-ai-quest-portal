...
test('Step 5A demo identity switching remains API-confirmed and fail-safe', async ({ page }) => {
  ...
  await expect(page.getByRole('navigation', { name: 'Primary navigation' }).getByRole('button', { name: 'Review Queue', exact: true })).toHaveCount(0)
  ...
  await expect(page.getByLabel('Active identity').getByText('Synthetic Manager')).toBeVisible()
  await expect(page.getByRole('heading', { name: 'Dashboard', level: 1 })).toBeVisible()
  await expect(page.getByRole('navigation', { name: 'Primary navigation' }).getByRole('button', { name: 'Review Queue', exact: true })).toBeVisible()
  await expect(page.getByLabel('Active identity').getByText('Manager', { exact: true })).toBeVisible()
