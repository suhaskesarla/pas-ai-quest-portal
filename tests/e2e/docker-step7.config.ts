...
process.env.QA_SCREENSHOT_DIR = path.join(runRoot, 'docker-step7')
process.env.QA_TEST_MODE = 'Docker Compose Step 7'
...
export default defineConfig({
  testDir: '.',
  testMatch: 'docker-step7.spec.ts',
  timeout: 60_000,
  workers: 1,
  fullyParallel: false,
  reporter: [['list'], ['./evidence-reporter.ts']],
  outputDir: path.join(runRoot, 'docker-step7', 'artifacts'),
