...
process.env.QA_SCREENSHOT_DIR = path.join(runRoot, 'docker-full-demo')
process.env.QA_TEST_MODE = 'Clean Docker canonical full demo'
...
export default defineConfig({
  testDir: '.',
  testMatch: 'demo/docker-full-demo.spec.ts',
  timeout: 300_000,
  workers: 1,
  fullyParallel: false,
  reporter: [['list'], ['./evidence-reporter.ts']],
  outputDir: path.join(runRoot, 'docker-full-demo', 'artifacts'),
  use: {
