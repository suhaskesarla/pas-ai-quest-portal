...
process.env.QA_SCREENSHOT_DIR = path.join(runRoot, 'docker-manager-scoresheet')
process.env.QA_TEST_MODE = 'Docker Compose Manager Scoresheet'
...
export default defineConfig({
  testDir: '.',
  testMatch: 'docker-manager-scoresheet.spec.ts',
  timeout: 90_000,
  workers: 1,
  fullyParallel: false,
  reporter: [['list'], ['./evidence-reporter.ts']],
  outputDir: path.join(runRoot, 'docker-manager-scoresheet', 'artifacts'),
